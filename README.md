...
- **Whole-server backups** — every database on a connection as its own `pg_dump --format=custom` archive, plus the roles, passwords and tablespaces `pg_dump` cannot see, bundled into one tar. PostgreSQL has no single-file equivalent (`pg_dump` covers one database, `pg_dumpall` emits only plain SQL), so the bundle is what makes "one file" and "restores with `pg_restore`" true at once. Restore is selective: pick the databases, choose whether to apply the globals, and target any registered server — not only the one it came from.
...
- `npm test` — unit tests across both workspaces (194): server-side crypto, the SQL script lexer, the filter builder, CSV parsing and Telegram delivery; web-side URL filter codec, SQL read/write classification, `EXPLAIN` plan parsing, cron building and formatting; plus the metadata snapshot/restore round-trip, DSN assembly/parsing (escaping, IPv6, defaults) the `.env` discovery/reader/writer, a connection round-trip through snapshot/restore that pins down both the success case and the changed-APP_SECRET failure, and the ustar writer/reader behind whole-server bundles — including a cross-check that GNU tar reads the archives it produces. Pure modules only — no DOM, so the suite stays fast.
